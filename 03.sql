/*
  ...
  - 관계형(Relational) 데이터베이스
    ...
    2. 데이터를 테이블 형태로 저장하고 여러테이블을 조합하여 비즈니스S 관계를 도출하는 구조
    3. 데이터의 중복을 최소화할 수 있으며 사용하기 편리하고 데
    이터의 무결성, 트랙잭션 처리 등 데이터베이스 관리 시스템으로 뛰어난 성능을 보여준다.
...
/* 컬럼의 산술 연산
   ...
   */
   --EMPLOYEE 테이블에서 직원명(EMP_NAME), 직원의 연봉SALARY*12 조회
   
   SELECT EMP_NAME, SALARY, SALARY * 12
   FROM EMPLOYEE;
   
   
    -- EMPLOYEE 테이블에서 직원명, 보너스가 포함된 연봉(급여 + (보너스 *급여) * 12)
    SELECT EMP_NAME, (SALARY+ (SALARY * BONUS)) * 12
    FROM EMPLOYEE;   
    
   -- EMPLOYEE 사원명 입사일 근무일 수 (오늘날짜 - 입사일)
   -- 오늘 날짜 : SYSDATE
   ...
   */
  -- EMPLOYEE 테이블에서 직원명, 연봉, 보너스가 포함된 연봉((급여 + (보너스*급여))*12
   SELECT 
   EMP_NAME AS 직원명,
   SALARY*12 AS "연봉", 
  (SALARY+(BONUS*SALARY)) * 12 "총 소득(원)"
   ...
   /*
   리터럴 
   - SELECT절에 리터럴을 사용하면 테이블에 존재하는 데이터처럼 조회가 가능
   - 즉, 리터럴은 RESULT SET의 모든 행에 반복적으로 출력된다.
   
   */
   -- EMPLOYEE 테이블 사원 직원명 급여, 단위(원) 조회
   SELECT 
   EMP_ID "사번",
   EMP_NAME "직원명",
   SALARY "급여",
   ...
   */
   ...
   --DISTINCT는 SELECT 절에 딱 한 번만 기술 가능
   ...
   */
   ...
   --EMPLOYEE에서 부서코드가 D1인 사원들의 사원명 급여 부서코드만 조회
   ...
   --급여가 400만 원 이상인 사원들의 사원명, 부서코드, 급여 조회
   SELECT EMP_NAME, DEPT_CODE, SALARY
   FROM EMPLOYEE
   WHERE SALARY >= 4000000;
   
   --재직중인 사원들의 사번, 사원명, 입사일
   ...
   --부서코드가 'D6'이거나 급여가 300만원 이상인 사원명, 부서코드, 급여
   SELECT EMP_NAME, DEPT_CODE, SALARY
   FROM EMPLOYEE
   WHERE DEPT_CODE = 'D6' OR SALARY >= 3000000;
   
   --급여가 350이상 600이하 받는 사원명, 사번, 급여
   ...
   /*
    ...
    - 비교하고자하는 컬럼값이 내가 제시한 특정 패턴에 만족될 경우 조회
     [표현법]
      ...
      비교대상컬럼 LIKE '_문자_' => 비교대상컬럼값에 문자앞에 무조건 한글자씩 올 경우 조회
   ...
   */
   ...
   -- 이름 중에 하 가 포함된 사원들의 사원명, 주민번호, 전화번호 
   SELECT EMP_NAME, EMP_NO, PHONE
   FROM EMPLOYEE
   WHERE EMP_NAME LIKE '%하%';
   
   --이름 가운데 글자가 하인 사원들의 사원명 전화번호 조회
   SELECT EMP_NAME, PHONE
   FROM EMPLOYEE
   WHERE EMP_NAME LIKE '_하_';
   
   --전화번호 3번째 자리가 1인 사원들의 사번, 사원명, 전화번호, 이메일
   ...
   */
   ...
   --부서 배치를 아직 받지 않고 보너스는 받는 사원들의 이름, 보너스, 부서코드
...
*/
   ...
   */
   ...
   --직급코드가 J7이거나 J2인 사원들 중 급여가 200이상인 모든 컬럼 조회
   SELECT *
   FROM EMPLOYEE
   WHERE JOB_CODE IN ('J7', 'J2') AND SALARY >= 2000000;
   
   --1. 사수가 없고 부서배치도 받지 않은 사원들의 사원ㄴ명, 사수사번, 부서코드 조회
    SELECT EMP_NAME, MANAGER_ID, DEPT_CODE
    FROM EMPLOYEE
    WHERE MANAGER_ID IS NULL AND DEPT_CODE IS NULL;
   
   --2. 연봉(보너스 포함x) 3000만원 이상이고 보너스를 받지 않은 사람들의 사번, 사원명, 급여 보너스 조회
    SELECT EMP_ID, EMP_NAME, BONUS
    FROM EMPLOYEE
    WHERE (SALARY * 12) >= 30000000 AND BONUS IS NULL;
   
   --3. 입사일이 950101이상이고 부서배치를 받지 않은 사원들의 사번, 사원명, 입사일, 부서코드 조회
    SELECT EMP_ID, EMP_NAME, HIRE_DATE, DEPT_CODE
    FROM EMPLOYEE
    WHERE HIRE_DATE >= '95/01/01' AND DEPT_CODE IS NULL;
  
   --4. 급여가 200이상 500이하이고 입사일이 010101 이상이고 보너스를 받지 않는 사원들의 사번, 사원명, 급여, 입사일, 보너스 조회
    SELECT EMP_NAME, SALARY,HIRE_DATE, BONUS
    FROM EMPLOYEE
    WHERE (SALARY BETWEEN 2000000 AND 5000000) AND HIRE_DATE >= '01/01/01' AND BONUS IS NULL;
   
   --5. 보너스 포함 연봉 NULL 아니고 이름이 하가 포함되어 있는 사원들의 사번, 사원명, 급여, 보너스포함연봉 조회(별칭)
    ...
    ORDER BY 절
     ...
     - 실행 순서 : FROM절 -> WHERE 절 -> SELECT 절 -> ORDER BY 절
     [표현법]
      SELECT 컬럼, 컬럼, ...
      FROM 테이블명
      WHERE 조건식
