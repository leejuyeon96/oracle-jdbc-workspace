/*
    DML(DATA MANIPULATION LANGUAGE)
     : 데이터 조작 언어로 테이블에 값을 삽입INSERT 하거나 수정UPDATE 하거나 삭제 DELETE 하는 구문이다.
    ...
    - 테이블에 새로운 행을 추가하는 구문
      [표현법]
        ...
        2) INSERT INTO 테이블명(컬럼명, 컬럼명, ... , 컬럼명) VALUES(값, 값, 값...., 값);
            ...
            - NOT NULL 조건일땐 무조건 값을 넣어야 함
            - 기본값(DEFAULT)이 지정되어 있으면 기본값이 들어감
        3) INSERT INTO 테이블명 (서브쿼리);
            - VALUES 대신 서브쿼리로 조회한 결과값을 통째로 INSERT
            - 서브쿼리의 결과값이 ISNERT문에 지정된 테이블 컬럼의 개수와 데이터 타입이 같아야 함
...
INSERT INTO EMP_01
VALUES(300, '유아지원팀', '공유', DEFAULT); --순서 중요

--컬럼순서와 데이터 타입이 맞지 않아서 에러 발생
INSERT INTO EMP_01
...
--표현법 3)
--EMPLOYEE에서 직원의 사번, 이름, 부서명, 입사일 조회 EMP_01 INSERT 하시오 
...
/*
 ...
 INSERT ALL 을 이용해서 여러 테이블에 한 번에 데이터 삽입이 가능
 [표현법]
 1)INSERT ALL
...
*/
-- 표현법 1)
CREATE TABLE EMP_DEPT AS SELECT EMP_ID, EMP_NAME, DEPT_CODE, HIRE_DATE 
FROM EMPLOYEE
WHERE 1=0;

CREATE TABLE EMP_MANAGER AS SELECT EMP_ID, EMP_NAME, MANAGER_ID 
FROM EMPLOYEE 
WHERE 1=0;
...
--EMP_DEPT 테이블에 EMPLOYEE 테이블의 부서코드가 D1인 직원의 사번, 이름, 부서코드, 입사일 삽입 
...
--EMPLOYEE 테이블의 입사일 기준으로 2000년 1/1 이전 입사사원 정보는 OLD 삽입 이후는 NEW
INSERT ALL WHEN HIRE_DATE < '2000/01/01' THEN 
...
/*
    ...
    SET 컬럼명 = 변경하려는값 OR 서브쿼리
      ...
      -UPDATE시 서브쿼리를 사용해서 서브쿼리를 수행한 결과값으로 컬럼 값을 변경할 수있다.
*/
...
--EMP_SALARY에서 선동일 사장 급여 7000 보너스 0.2
UPDATE EMP_SALARY SET SALARY = 7000, BONUS = 0.2
WHERE EMP_NAME = '선동일';

--모든사원의급여를 기존 급여에서 10프로 인상한급여로 변경 1.1
UPDATE EMP_SALARY SET SALARY = SALARY*1.1;

UPDATE EMP_SALARY SET EMP_ID = NULL --NOT NULL 제약조건에 위배된다.
WHERE EMP_ID = 200;

UPDATE EMP_SALARY SET DEPT_CODE = 'DO' --FOREIGN KEY 제약조건에 위배된다.
WHERE EMP_NAME = '노옹철';

--UPDATE시 서브쿼리 사용 가능
-- 방명수 사원의 급여와 보너스를 유재식 사원과 동일하게
...
*/
--EMP_SALARY 테이블에서 선동일 사장 데이터 지우기
...
--EMP_SALARY 테이블에서 DEPT_CODE가 D5
DELETE FROM EMP_SALARY
WHERE DEPT_CODE = 'D5';

--DEPARTMENT 테이블에서 DEPT_ID가 D1
...
DROP TABLE DEPT_COPY; --테이블까지 지워버리기
