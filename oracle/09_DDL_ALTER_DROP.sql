...
--ALTER DDL 은 객체 수정 테이블을 바꾸는 거임 값을 바꾸는게 아님
...
ALTER TABLE DEPT_COPY DROP COLUMN COLUMN CNAME;
ALTER TABLE DEPT_COPY DROP COLUMN COLUMN LNAME; --최소 한개의 컬럼은 존재해야해서
--ROLLBACK; 은 데이터 관련이라 안됨
 ROLLBACK;
...
--참조되고 있는 부모 테이블은 하부로 삭제가 되지 않는다.
DROP TABLE MEMBER_GRADE;
--만약 삭제하고자 한다면 방법1 자식 테이블을 먼저 삭제한 후 부모 테이블 삭제 (권장)
DROP TABLE MEMBER;
DROP TABLE MEMBER_GRADE;


--                   방법2 그냥 부모 테이블만 삭제하는데 제약조건까지 같이 삭제하는 방법
