...
- SQL 문장 내에서 변수의 정의, 조건처리IF, 반복처리LOOP, FOR, WHILE 등을 지원하여 SQL의 단점 보완
...
- 실행부(EXCUTABLE SECTION) : BEGIN 으로 시작, SQL문 또는 제어문(조건문, 반복문)등의 로직을 기술하는 부분
- [예외처리부(EXCEPTION SECTION)] : EXCEPTION 으로 시작, 예외발생시 해결하기 위한 구문을 미리 기술해둘 수 있는 부분
*/
-- 출력기능 활성화
SET SERVEROUTPUT ON;
...
[표현식]
변수명 [CONSTANT] 자료형 [:=값]; --CONSTANT

*/
DECLARE EID NUMBER; ENAME VARCHAR2(20);
PI CONSTANT NUMBER := 3.14;

BEGIN EID := &번호; ENAME := '&이름';
...
DECLARE EID EMPLOYEE.EMP_ID%TYPE;
ENAME EMPLOYEE.EMP_NAME%TYPE;
SAL EMPLOYEE.SALARY%TYPE;
BEGIN SELECT EMP_ID, EMP_NAME, SALARY
INTO EID, ENAME, SAL
FROM EMPLOYEE
WHERE EMP_NAME = '&이름';
...
/
--문제로 풀어보기!
--레퍼런스 타입의 변수로 EID, ENAME, JCODE, DTITLE, SAL 선언하고 각 변수의 자료형은  EMPLOYEE 테이블의 EMP_ID, EMP_NAME, JOB_CODE, SALARY 컬럼과 
--DEPARTMENT 테이블의 DEPT_TITLE 컬럼의 자료형 참조 사용자가 입력한 사번과 일치하는 사원을 조회한후 조회 결과 출력
...
BEGIN SELECT * INTO EMP FROM EMPLOYEE
WHERE EMP_NAME = '&직원명';
...
2.실행부
 ...
 1-1) IF 구문
*/
--사번을 입력받은 후 해당 사원의 사번EID, 이름ENAME, 급여SAL, 보너스BONUS를 출력
--단, 보너스를 받지 않는 사원은 보너스 출력전에 보너스를 지급받지 않는 사원입니다 라는 문구 출력
...
END;
/
-- 사용자에게 입력받은 사원과 일치하는 사원의 급여 조회 후 출력 GRADE : VARCHAR2(10) 
-- (조회한 급여는 SAL 변수에 대입) 500만원이상이면 고급 300 중급 300 미만 초급 출력은 해당사원의 급여 등급은 고급입니다 와 같이

DECLARE 
SAL EMPLOYEE.SALARY%TYPE;
GRADE VARCHAR2(10);
BEGIN SELECT SALARY := '&급여';
INTO SAL
FROM EMPLOYEE
WHERE EMP_ID = '&사번';
...
END;
*/
--사번을 입력받은 후 사원의 모든 컬럼 데이터를 EMP 에 대입하고 DEPT CODE 에 따라서 알맞는 부서 출력
--D1 인사 관리부 D2 회계관리부 D3 마케팅부 D4 국내영업부 D5 해외영업1부 D6 해외영업2부 D7 해외영업 3 D8 기술지원부 D9 총무부 나머지 부서없음
DECLARE
...
DECLARE 
NUM NUMBER := 1;
BEGIN LOOP DBMS_OUTPUT.PUT_LINE(NUM);
NUM := NUM + 1;
IF (NUM > 5) THEN EXIT;
END IF;
END LOOP;
...
BEGIN 
  ...
  /
 DROP TABLE TEST;
 CREATE TABLE TEST(NUM NUMBER, CREATE_DATE DATE);
...
/
--구구단 2~9단
DECLARE DAN NUMBER := 2; SU NUMBER;
BEGIN
    WHILE DAN <= 9 LOOP 
        SU := 1;
       WHILE SU <= 9 LOOP
...
3.예외처리부
 ...
 EXCEPTION 
    ...
    *오라클에서 미리 정의 되어있는 시스템 예외
    - NO_DATA_FOUND : SELECT 한 결과가 한 행도 없을 경우
    - TOO_MANY_ROWS : SELECT한 결과가 한 행이 리턴되어야하는데 여러 행일 경우
...
EXCEPTION WHEN ZERO_DIVIDE THEN DBMS_OUTPUT.PUT_LINE('나누기 연산시 0으로 나눌 수 없습니다');
END;
/
--UNIQUE 제약조건 위배 시..
...
--TOO_MANY_ROWS : SELECT 한 결과가 여러 행인 경우 (한 행이 리턴되어야 하는 경우)
DECLARE EID EMPLOYEE.EMP_ID%TYPE;
DCODE EMPLOYEE.DEPT_CODE%TYPE;
BEGIN SELECT EMP_ID, DEPT_CODE
INTO EID, DCODE
FROM EMPLOYEE
...
EXCEPTION WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('조회 결과가 없습니다');
