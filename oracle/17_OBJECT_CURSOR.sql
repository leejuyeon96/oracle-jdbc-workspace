...
1. 묵시적 IMPLICIT 커서
    ...
    PL/SQL에서 SQL 문 실행 시마다 자동으로 만들어져서 사용된다.
...
2. 명시적 EXPLICIT 커서
    ...
    *커서 속성
     ...
     커서명%ISOPEN : 커서가 OPEN 상태인경우 TRUE , 아니면 FALSE
...
--2. 명시적 커서
-- PL/SQL 을 사용해서 급여가  300 이상 사번 EID, 이름 ENAME , 급여 SAL
    ...
    END;
    /
-- 전체 부서에 대해 부서코드, 부서명, 지역조회 CURSOR, PL/SQL
...
END;
/
--FOR LOOP 를 이용한 커서 사용
--1) LOOP 시작 시 자동으로 커서를 OPEN
