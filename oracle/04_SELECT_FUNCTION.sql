...
--사원명, 사원명의 글자 수, 사원명의 바이트 수, 이메일, 이메일 글자 수, 이메일 바이트 수
...
--'S'가 포함되어있는 이메일 중 이메일, 이메일의 @위치, 이메일 뒤에서 두번째에 있는 S 위치
...
/*
LPAD/ RPAD
-문자열을 조회할 때 통일감있게조회하고자 할 때 사용
-LPAD/RPAD(STRING, 최종적으로 반환할 문자의 길이,[덧붙이고자하는문자]
-문자열에 덧붙이고자하는 문자를 왼쪽 또는 오른쪽에 덧붙여서 최종 N길이 만큼 문자열을 반환
...
--직원명, 입사년도, 입사월, 입사일 조회
SELECT EMP_NAME, 
EXTRACT(YEAR FROM HIRE_DATE)"입사년도", EXTRACT(MONTH FROM HIRE_DATE)"입사월", EXTRACT(DAY FROM HIRE_DATE)"입사일" 
FROM EMPLOYEE
...
--직원명, 급여, 연봉
SELECT EMP_NAME, TO_CHAR(SALARY, 'L99,999,999'), TO_CHAR((SALARY * 12), 'L99,999,999')"연봉"
FROM EMPLOYEE
ORDER BY "연봉";
...
ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD HH24:MI:SS';
 ...
 SELECT TO_DATE('20230621 033823', 'YYYYMMDD HHMISS') FROM DUAL;
 /*
 TO_NUMBER('문자값', [포맷])
 -문자형 데이터를 숫자 타입으로 변환해서 반환
 */
SELECT '1000000' + '550000' FROM DUAL;
SELECT '1,000,000' + '550,000' FROM DUAL;--ERROR
...
*/
 ...
 /*
 NVL2(값1, 값2, 값3)
 - 값1이 NULL이 아니면 값2 값1이 NULL이면 값3
 
 */
 --사원명, 부서코드 있는경우 부서있음 없는 경우 부서없음
 SELECT EMP_NAME, NVL2(DEPT_CODE, '부서있음', '부서없음')
 FROM EMPLOYEE;
 
 /*
 NULLIF(값1, 값2)
 - 두 개의 값이 동일하면 NULL을 반환하고 두개의 값이 동일하지 않으면 값1 반환
 */
 ...
 */
 ...
 --직급 코드가 j7인 사원은 급여를 10%인상 j6 15 j5 20 그외의 직급 사원 5
 --정렬 직급 코드 j1부터 인상된 급여 높은 순
...
--사원명, 급여, 급여 등급(1~4) 급여값이 500초과 1 500이하 350초과 2, 350이하 200초과 3 그외 4
...
*/
--전체 사원의 평균 급여 조회
SELECT TO_CHAR(ROUND(AVG(SALARY)), 'FM99,999,999') "평균급여"
FROM EMPLOYEE;
