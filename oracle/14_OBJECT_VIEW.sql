...
FROM EMPLOYEE E, DEPARTMENT D, NATIONAL
...
--UPDATE
--200번 사원의 연봉을 8000만 으로
UPDATE VW_EMP_SAL SET 연봉 = 80000000
WHERE EMP_ID = 200;-- 산술연산으로 정의된 컬럼은 데이터 변경 불가능
--200 급여  700만
UPDATE VW_EMP_SAL SET SALARY = 7000000
...
--4. 그룹함수나 GRUOP BY 절을 포함한 경우 부서별 급여의 합계, 평균 VW_GROUPDEPT
...
--UPDATE(에러)
UPDATE VW_GROUPDEPT SET 합계 = 8000000
WHERE DEPT_CODE = 'D1';
...
--5. DISTINCT 구문이 포함된 경우
CREATE RO REPLACE VIEW VW_DT_JOB
AS SELECT DISTINCT JOB_CODE
FROM EMPLOYEE;

SEKECT * FROM VW_DT_JOB;

--INSERT
INSERT INTO VW_DT_JOB VALUES('J8');

--UPDATE
UPDATE VW_DT_JOB SET JOB_CODE = 'J8'
WHERE JOB_CODE = 'J7';
...
--UPDATE
UPDATE VW_JOINEMP SET EMP_NAME = '서동일'
WHERE EMP_ID = 200;

UPDATE VW_JOINEMP SET DEPT_TITLE = '회계부'
WHERE EMP_ID = 200;--에러
...
-- VIEW 옵션
 -- 2. FORCE | NOFORCE
 -- NOFORCE(기본값): 서브쿼리에 기술된 테이블이 존재하는 테이블이어야만 VIEW가 생성되게 함
 DROP TABLE TEST;
 -- 안되는 게 정상!! NOFORCE는 테이블 생성 후 만들기 가능
 ...
 AS SELECT * FROM TEST;
 --> 컴파일 오류와 함게 뷰가 생성됨, 테이블이 생성되면 오류가 사라짐!
 SELECT * FROM VW_EMP;
 ...
 UPDATE VW_EMP
 SET SALARY = 2000000
 WHERE EMP_ID = 200; --> 에러! 위 서브쿼리의 조건에 부합되지 않기때문에 변경 안 됨!
 UPDATE VW_EMP
 SET SALARY = 4000000
 WHERE EMP_ID = 200; --> 성공! 서브쿼리에 기술한 조건에 부합되기때문에 변경 가능!
 SELECT * FROM EMPLOYEE;
 ROLLBACK;
