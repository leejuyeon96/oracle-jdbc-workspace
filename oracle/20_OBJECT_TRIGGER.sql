/*
    ...
    - 신고횟수가 일정 수를 넘었을 때 해당회원을 자동으로 블랙리스트로 처리
    -입출고에 대한 데이터가 기록INSERT 될 때마다 해당 상품에 대한 재고수량을 매번 수정UPDATE 해야 될 때
    ...
    *트리거 종류 
    ...
    - SQL 문에 의해 영향을 받는 각 행에 따른 분류
        >STATEMENT TRIGGER 문장트리거 : 이벤트가 발생한 SQL 문에 대해 딱 한 번만 트리거 실행
        >ROW TRIGGER 행트리거 : 해당 SQL 문 실행할 때마다 매번 트리거 실행(FOR EACH ROW 옵션 기술해야됨)
...
*/
--1. 문장트리거
-- EMPLOYEE 테이블에 새로운 행이 INSERT 될 때 신입사원이 입사했습니다 메시지 자동 출력
...
UPDATE EMPLOYEE SET DEPT_CODE = 'D3'
WHERE DEPT_CODE = 'D6';
...
--TB_PRODUCT 테이블에 매번 자동을 재고 수량 UPDATE 되게끔 트리거 정의
