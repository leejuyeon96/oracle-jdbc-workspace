/*
    함수 FUNCTION
    - 프로시저와 거의 유사한 용도로 사용하지만 실행결과를 되돌려 받을 수 있다는 점이 다르다.
    ...
    *실행
    EXECUTE(또는 EXEC) 함수명 [(매개값1, 매개값 2)];
    
    *삭제
    DROP FUNCTION 함수명;
*/
--사번V_EMP_ID 을 입력받아 해당 사원의 보너스V_BONUS를 포함하는 연봉을 계산하고 리턴NUMBER하는 함수 생성
CREATE FUNCTION BONUS_CALC
    ...
    RETURN (V_SALARY + (V_BONUS*V_SALARY))*12;
    END;
    /
-- 함수 호출
...
--EMPLOYEE 테이블에서 방금 만든 함수 BONUS_CALC 이용해서 보너스 포함 연봉 4000이상 사원 사번, 직원명, 급여, 보너스 조회
